...
def mediapipe_video_analysis(file_path):
    ...
    p = Path(file_path)
    parent_folder=((p.parent).parent).parent
    subfolder_name = "ViconMarkerless2Opensim/uploads"
    output_folder = parent_folder / subfolder_name
    fileout = output_folder / (p.stem + '.csv')
    # %%%
